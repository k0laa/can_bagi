...
MeshAid, internet ve GSM bağlantısının kesildiği afet senaryolarında acil durum haberleşmesini sağlamak için tasarlanmış bir prototip sistemdir. Projede ESP32 tabanlı mesh ağı, Python/FastAPI backend, Flutter mobil uygulama ve React web dashboard birlikte çalışır.
...
3. **Backend (`backend`)**
    ...
    - SQLite veritabanı kullanır (`meshaid.db`)
...
| Donanım  | ESP3 - painlessMesh - AsyncWebServer - ArduinoJson         |
| Auth     | JWT Token                                                  |
...
- `firmware/`
    ...
    - `node_firmware/` : sahada çalışan ESP32 node kodu

## Nasıl Çalışır?

1. Mobil uygulama veya doğrudan ESP32 node HTTP isteği ile SOS/request gönderir.
2. Node mesajı esp32 wifi mesh üzerinden gateway'e iletir.
...
1. `frontend/web` dizinine gidin
2. `npm install`
3. `npm run dev`

### Mobil Uygulama

1. `frontend/mobile` dizinine gidin
2. `flutter pub get`
